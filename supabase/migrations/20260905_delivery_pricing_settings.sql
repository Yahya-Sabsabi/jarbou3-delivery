...
create table if not exists public.delivery_pricing_settings (
  ...
  per_minute bigint not null default 1 check (per_minute >= 0 and per_minute <= 1000000000),
  currency_code text not null default 'SYP_NEW' check (currency_code = 'SYP_NEW'),
  updated_at timestamptz not null default now(),
  updated_by uuid references public.users(id)
);

insert into public.delivery_pricing_settings (singleton, minimum_fare, per_km, per_minute, currency_code)
values (true, 60, 25, 1, 'SYP_NEW')
