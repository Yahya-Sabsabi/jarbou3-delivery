...
function mapError(target, text = "تعذر تحميل الخريطة. تحقق من الاتصال ثم أعد المحاولة.") { if (!target) return; target.querySelector(".map-empty")?.remove(); target.insertAdjacentHTML("beforeend", `<p class="map-empty">${fleetEscape(text)}</p>`); }
function mapReadyMessage(target, text) { if (!target) return; target.querySelector(".map-empty")?.remove(); target.insertAdjacentHTML("beforeend", `<p class="map-empty map-status">${fleetEscape(text)}</p>`); }
...
function createAdminMap(target, onLoad, onFailure) {
  ...
  try {
    ...
    failureTimer = window.setTimeout(fail, 3000);
    return map;
...
function createAdminLeafletMap(target, onLoad) {
  ...
  target.classList.add("map-mode-leaflet");
  const map = window.L.map(target, { zoomControl: false, attributionControl: true, minZoom: 11, maxZoom: 19, maxBounds: [[35.04, 36.60], [35.23, 36.91]], maxBoundsViscosity: 1, doubleClickZoom: true, zoomAnimation: true, fadeAnimation: true, markerZoomAnimation: true }).setView([ADMIN_HAMA_CENTER[1], ADMIN_HAMA_CENTER[0]], 12);
...
function installLeafletFleetMap(payload) {
  ...
  if (!leafletReady()) { mapReadyMessage(target, "جارٍ تجهيز محرك الخريطة…"); window.setTimeout(() => { if (state.currentView === "fleet" && document.getElementById("fleet-map") === target) window.installFleetOperationsMap?.(fleetPayload); }, 250); return; }
...
window.installFleetOperationsMap = function installFleetOperationsMap(payload) {
  ...
  if (!fleetOperationsMap || fleetMapTarget !== target) { destroyVectorFleetMap(); fleetMapTarget = target; target.innerHTML = ""; fleetOperationsMap = createAdminMap(target, () => renderFleetPayload(fleetPayload), () => { destroyVectorFleetMap(); installLeafletFleetMap(fleetPayload); }); if (!fleetOperationsMap) installLeafletFleetMap(payload); }
...
function installLeafletPlacesMap(places, latitudeInput, longitudeInput) { const target = document.querySelector("#places-map"); if (!target) return; if (!leafletReady()) { mapReadyMessage(target, "جارٍ تجهيز محرك الخريطة…"); window.setTimeout(() => { if (state.currentView === "places" && document.getElementById("places-map") === target) window.installPlacesMap?.(places); }, 250); return; } destroyVectorPlacesMap(); if (!leafletPlacesMap || leafletPlacesTarget !== target) { destroyLeafletPlacesMap(); leafletPlacesTarget = target; target.innerHTML = ""; leafletPlacesMap = createAdminLeafletMap(target, () => { bindPlacesClick(leafletPlacesMap, true, latitudeInput, longitudeInput); syncLeafletPlacesMarkers(places); }); } else { leafletPlacesMap.invalidateSize(); syncLeafletPlacesMarkers(places); } }
...
window.installPlacesMap = function installPlacesMap(places = []) { const target = document.querySelector("#places-map"); if (!target) return; const latitudeInput = document.querySelector("#place-latitude"); const longitudeInput = document.querySelector("#place-longitude"); if (!maplibreSupported()) { installLeafletPlacesMap(places, latitudeInput, longitudeInput); return; } destroyLeafletPlacesMap();   if (!maplibreReady(target)) { installLeafletPlacesMap(places, latitudeInput, longitudeInput); return; } if (!placesMap || placesMapTarget !== target) { destroyVectorPlacesMap(); placesMapTarget = target; target.innerHTML = ""; placesMap = createAdminMap(target, () => { bindPlacesClick(placesMap, false, latitudeInput, longitudeInput); syncPlacesMarkers(places); }, () => { destroyVectorPlacesMap(); installLeafletPlacesMap(places, latitudeInput, longitudeInput); }); if (!placesMap) installLeafletPlacesMap(places, latitudeInput, longitudeInput); } else if (placesMap._optimusLoaded) { placesMap.resize(); syncPlacesMarkers(places); } };

window.disposeAdminMaps = function disposeAdminMaps() { destroyVectorFleetMap(); destroyLeafletFleetMap(); destroyVectorPlacesMap(); destroyLeafletPlacesMap(); };
window.addEventListener("resize", () => { if (fleetOperationsMap?._optimusLoaded) fleetOperationsMap.resize(); if (leafletFleetMap) leafletFleetMap.invalidateSize(); if (placesMap?._optimusLoaded) placesMap.resize(); if (leafletPlacesMap) leafletPlacesMap.invalidateSize(); });
document.addEventListener("visibilitychange", () => { if (document.visibilityState !== "visible") return; window.setTimeout(() => { if (fleetOperationsMap?._optimusLoaded) fleetOperationsMap.resize(); if (leafletFleetMap) leafletFleetMap.invalidateSize(); if (placesMap?._optimusLoaded) placesMap.resize(); if (leafletPlacesMap) leafletPlacesMap.invalidateSize(); }, 80); });
