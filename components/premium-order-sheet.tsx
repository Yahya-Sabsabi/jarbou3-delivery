...
function LocationRow({ icon, title, value, active, onPress }: { icon: "radio-button-checked" | "location-on"; title: string; value: string; active: boolean; onPress: () => void }) {
  return <Pressable onPress={onPress} style={({ pressed }) => [styles.locationRow, active && styles.locationRowActive, pressed && styles.pressed]}>
    <View style={[styles.locationIcon, active && styles.locationIconActive]}><MaterialIcons name={icon} size={17} color={active ? "#EA580C" : "#83918B"} /></View>
...
export function PremiumOrderSheet({ source, destination, selecting, onSelectMode, locationNotice, onDismissNotice, addressQuery, onAddressQueryChange, searchingAddress, onSearch, addressResults, onChooseAddress, favoriteAddresses, onChooseFavorite, onDeleteFavorite, favoriteLabel, onFavoriteLabelChange, onSaveFavorite, savingFavorite, discountCode, onDiscountCodeChange, onVerifyDiscount, discountFetching, discountExpanded, onToggleDiscount, onOpenFavorites, appliedDiscount, route, routeLoading, payment, onPaymentChange, onSubmit, submitting }: PremiumOrderSheetProps) {
  ...
  return <View style={styles.container}>
    ...
    <View style={styles.quickRow}>
      <Pressable onPress={onToggleDiscount} style={({ pressed }) => [styles.quickAction, pressed && styles.pressed]}><MaterialIcons name="local-offer" size={18} color="#EA580C" /><Text style={styles.quickText}>كود خصم</Text></Pressable>
      <Pressable onPress={onOpenFavorites} style={({ pressed }) => [styles.quickAction, pressed && styles.pressed]}><MaterialIcons name="bookmark-border" size={19} color="#EA580C" /><Text style={styles.quickText}>عناويني</Text></Pressable>
    </View>
    ...
    <View style={styles.favoriteSave}><View style={styles.sectionHeader}><Text style={styles.sectionLabel}>حفظ الموقع الحالي</Text><MaterialIcons name="bookmark-add" size={18} color="#EA580C" /></View><View style={styles.discountRow}><TextInput value={favoriteLabel} onChangeText={onFavoriteLabelChange} onSubmitEditing={() => { Keyboard.dismiss(); onSaveFavorite(); }} returnKeyType="done" placeholder="مثال: المنزل" placeholderTextColor="#9AA49F" style={styles.discountInput} textAlign="right" /><Pressable onPress={onSaveFavorite} style={styles.lightButton}><Text style={styles.lightButtonText}>{savingFavorite ? "…" : "حفظ"}</Text></Pressable></View></View>
...
const styles = {
  ...
  statusDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: "#EA580C", shadowColor: "#EA580C", shadowOpacity: 0.35, shadowRadius: 6 },
  eyebrow: { color: "#A8A29E", fontSize: 11, fontWeight: "700", textAlign: "right" },
  title: { color: "#431407", fontSize: 18, fontWeight: "900", marginTop: 2, textAlign: "right" },
  price: { color: "#EA580C", fontSize: 16, fontWeight: "900" },
  ...
  metricLabel: { color: "#A8A29E", fontSize: 10, fontWeight: "700" },
  ...
  locationIconActive: { backgroundColor: "#FFEDD5" },
  locationTitle: { color: "#7D8983", fontSize: 10, fontWeight: "800", textAlign: "right" },
  locationValue: { color: "#431407", fontSize: 12, fontWeight: "800", textAlign: "right", marginTop: 2 },
  ...
  quickAction: { flexDirection: "row-reverse", alignItems: "center", gap: 7, paddingVertical: 10, paddingHorizontal: 13, borderRadius: 13, backgroundColor: "#FFF7ED" },
  quickText: { color: "#EA580C", fontSize: 11, fontWeight: "900" },
  detailsDivider: { height: 1, backgroundColor: "#E9EEEB", marginVertical: 2 },
  notice: { flexDirection: "row-reverse", alignItems: "center", justifyContent: "space-between", gap: 8, backgroundColor: "#FFF7ED", borderRadius: 13, padding: 11 },
  ...
  modePillActive: { backgroundColor: "#EA580C" },
  ...
  searchInput: { flex: 1, minHeight: 46, borderRadius: 14, backgroundColor: "#F5F7F6", paddingHorizontal: 13, color: "#431407", fontSize: 12, fontWeight: "700" },
  searchButton: { width: 46, minHeight: 46, borderRadius: 14, alignItems: "center", justifyContent: "center", backgroundColor: "#EA580C" },
  results: { gap: 6 },
  ...
  discountInput: { flex: 1, minHeight: 44, borderRadius: 12, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#FED7AA", paddingHorizontal: 11, color: "#431407", fontSize: 12, fontWeight: "800" },
  verifyButton: { minWidth: 64, minHeight: 44, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: "#EA580C" },
  verifyText: { color: "#FFFFFF", fontSize: 11, fontWeight: "900" },
  successText: { color: "#EA580C", fontSize: 10, fontWeight: "800", textAlign: "right" },
  paymentRow: { flexDirection: "row-reverse", gap: 8 },
  paymentPill: { flex: 1, alignItems: "center", paddingVertical: 11, borderRadius: 12, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#FED7AA" },
  paymentPillActive: { borderColor: "#EA580C", backgroundColor: "#E8F4EE" },
  paymentText: { color: "#64726A", fontSize: 11, fontWeight: "800" },
  paymentTextActive: { color: "#EA580C" },
  favoriteSave: { gap: 9, padding: 12, borderRadius: 16, backgroundColor: "#F7F9F8" },
  lightButton: { minWidth: 64, minHeight: 44, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: "#FFEDD5" },
  lightButtonText: { color: "#EA580C", fontSize: 11, fontWeight: "900" },
  confirmButton: { minHeight: 54, borderRadius: 17, paddingHorizontal: 15, flexDirection: "row-reverse", alignItems: "center", gap: 10, backgroundColor: "#EA580C", shadowColor: "#EA580C", shadowOpacity: 0.2, shadowRadius: 12, shadowOffset: { width: 0, height: 5 }, elevation: 4 },
...
export function PremiumMoreMenu({ visible, onClose, onDiscount, onFavorites }: { visible: boolean; onClose: () => void; onDiscount: () => void; onFavorites: () => void }) {
  ...
  return <View style={menuStyles.wrap} testID="premium-more-menu">
    <Pressable onPress={onDiscount} style={({ pressed }) => [menuStyles.item, pressed && menuStyles.itemPressed]}><MaterialIcons name="local-offer" size={18} color="#EA580C" /><Text style={menuStyles.itemText}>إضافة كود خصم</Text></Pressable>
    <Pressable onPress={onFavorites} style={({ pressed }) => [menuStyles.item, pressed && menuStyles.itemPressed]}><MaterialIcons name="bookmark-border" size={19} color="#EA580C" /><Text style={menuStyles.itemText}>العناوين المفضلة</Text></Pressable>
    <Pressable onPress={onClose} style={({ pressed }) => [menuStyles.item, pressed && menuStyles.itemPressed]}><MaterialIcons name="close" size={18} color="#A8A29E" /><Text style={menuStyles.itemTextMuted}>إغلاق</Text></Pressable>
  </View>;
}

const menuStyles = StyleSheet.create({
  wrap: { position: "absolute", top: 72, left: 16, zIndex: 40, minWidth: 190, padding: 7, borderRadius: 17, backgroundColor: "#FFFFFF", shadowColor: "#431407", shadowOpacity: 0.16, shadowRadius: 16, shadowOffset: { width: 0, height: 7 }, elevation: 10 },
  item: { flexDirection: "row-reverse", alignItems: "center", gap: 10, paddingHorizontal: 11, paddingVertical: 11, borderRadius: 12 },
  itemPressed: { backgroundColor: "#FFF7ED" },
  itemText: { flex: 1, color: "#431407", fontSize: 12, fontWeight: "800", textAlign: "right" },
  itemTextMuted: { flex: 1, color: "#A8A29E", fontSize: 11, fontWeight: "700", textAlign: "right" },
  favoritesWrap: { position: "absolute", top: 72, right: 16, zIndex: 41, width: 270, padding: 12, borderRadius: 18, backgroundColor: "#FFFFFF", shadowColor: "#431407", shadowOpacity: 0.16, shadowRadius: 16, shadowOffset: { width: 0, height: 7 }, elevation: 10 },
  ...
  favoritesTitle: { color: "#431407", fontSize: 13, fontWeight: "900", textAlign: "right" },
  ...
  favoriteCopy: { flex: 1 },
  favoriteLabel: { color: "#431407", fontSize: 11, fontWeight: "900", textAlign: "right" },
  favoriteAddress: { color: "#A8A29E", fontSize: 10, textAlign: "right", marginTop: 2 },
  emptyFavorites: { color: "#A8A29E", fontSize: 11, textAlign: "right", paddingVertical: 12 },
  addFavorite: { flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 6, paddingTop: 11 },
  addFavoriteText: { color: "#EA580C", fontSize: 11, fontWeight: "900" },
});
...
export function PremiumCustomerNav({ active, onHome, onOrders, onProfile }: { active: "home" | "orders" | "profile"; onHome: () => void; onOrders: () => void; onProfile: () => void }) {
  return <View style={navStyles.bar}>
    <Pressable onPress={onProfile} style={navStyles.item}><MaterialIcons name="person-outline" size={22} color={active === "profile" ? "#EA580C" : "#95A19B"} /><Text style={[navStyles.label, active === "profile" && navStyles.activeLabel]}>الحساب</Text></Pressable>
    <Pressable onPress={onOrders} style={navStyles.item}><MaterialIcons name="receipt-long" size={22} color={active === "orders" ? "#EA580C" : "#95A19B"} /><Text style={[navStyles.label, active === "orders" && navStyles.activeLabel]}>الطلبات</Text></Pressable>
    <Pressable onPress={onHome} style={navStyles.item}><MaterialIcons name="home-filled" size={22} color={active === "home" ? "#EA580C" : "#95A19B"} /><Text style={[navStyles.label, active === "home" && navStyles.activeLabel]}>الرئيسية</Text></Pressable>
  </View>;
}

const navStyles = StyleSheet.create({
  bar: { flexDirection: "row-reverse", alignItems: "center", justifyContent: "space-around", marginHorizontal: 16, marginTop: 10, paddingVertical: 9, paddingBottom: 10, borderRadius: 22, backgroundColor: "#FFFFFF", shadowColor: "#431407", shadowOpacity: 0.08, shadowRadius: 14, shadowOffset: { width: 0, height: 4 }, elevation: 4 },
  item: { minWidth: 76, alignItems: "center", gap: 3, paddingVertical: 3 },
  label: { color: "#95A19B", fontSize: 10, fontWeight: "800" },
  activeLabel: { color: "#EA580C" },
});
