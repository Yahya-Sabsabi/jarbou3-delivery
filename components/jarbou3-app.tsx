...
function Driver({ name, onTripActivity }: { name: string; onTripActivity: (active: boolean) => void }) {
  ...
  const declineOrder = trpc.jarbou3.declineOrder.useMutation({ onSuccess: () => { availableOrders.refetch(); Alert.alert("تم رفض العرض", "انتقل العرض تلقائياً إلى السفير التالي الأقرب."); }, onError: (error) => Alert.alert("تعذر رفض الطلب", error.message) });
...
export function Jarbou3App() {
  ...
  const submitOnboarding = trpc.jarbou3.submitOnboarding.useMutation({
    ...
    onError: (error) => Alert.alert("تعذر إرسال الطلب", error.message === "PHONE_ALREADY_REGISTERED" ? "هذا الرقم مستخدم بالفعل. اختر «لدي حساب بالفعل» لتسجيل الدخول أو استرجاع كلمة المرور." : error.message === "ONBOARDING_RATE_LIMITED" ? "تم إيقاف المحاولات مؤقتاً لحماية الحساب. حاول بعد قليل." : "راجع البيانات ثم حاول مرة أخرى."),
  });
