...
type DriverPage = "home" | "verify" | "drive" | "deliver" | "profile";
...
function Mark({ small = false }: { small?: boolean }) {
  return <View style={[styles.mark, small && styles.markSmall]}><Image source={require("@/assets/images/optimus-black-logo.png")} style={styles.markImage} contentFit="cover" /></View>;
}
...
function Customer({ name, phone, onTripActivity, onLogout }: { name: string; phone?: string; onTripActivity: (active: boolean) => void; onLogout: () => void }) {
  ...
  return <View style={styles.fill}><ScrollView contentContainerStyle={[styles.homeScroll, { paddingTop: Math.max(insets.top + 10, 22), paddingBottom: Math.max(insets.bottom + 104, 116) }]}><View style={styles.homeHeader}><View style={styles.homeIdentity}><Text style={styles.homeEyebrow}>OPTIMUS X · حماة</Text><Text style={styles.homeGreeting}>أهلاً، {name}</Text><Text style={styles.homeSubcopy}>جاهز لتوصيلك اليوم؟</Text></View><Mark small /></View><View style={styles.orderHero}><View style={styles.orderHeroGlow} /><Text style={styles.orderKicker}>الخدمة متاحة الآن</Text><Text style={styles.orderTitle}>إلى أين نوصلك اليوم؟</Text><Text style={styles.orderCopy}>حدد نقطة الاستلام والوجهة على الخريطة التفاعلية.</Text><Pressable onPress={() => { haptic(); setPage("order"); }} style={({ pressed }) => [styles.createOrderButton, pressed && styles.choosePressed]}><MaterialIcons name="near-me" size={19} color="#FFFFFF" /><Text style={styles.createOrderText}>إنشاء طلب توصيل</Text></Pressable><View style={styles.mapShortcut}><View style={styles.mapShortcutIcon}><MaterialIcons name="map" size={18} color="#EA580C" /></View><View style={styles.mapShortcutCopy}><Text style={styles.mapShortcutTitle}>الخريطة الكاملة داخل الطلب</Text><Text style={styles.mapShortcutText}>اختيار دقيق وسلس من أول لمسة</Text></View><MaterialIcons name="chevron-left" size={20} color="#A4B0AA" /></View></View><View style={styles.homeSectionHeader}><Text style={styles.homeSectionTitle}>آخر الطلبات</Text><Pressable onPress={() => navigateCustomer("orders")}><Text style={styles.homeSectionLink}>عرض الكل</Text></Pressable></View><View style={styles.recentEmpty}><View style={styles.recentIcon}><MaterialIcons name="receipt-long" size={20} color="#EA580C" /></View><View style={styles.recentCopy}><Text style={styles.recentTitle}>لا توجد طلبات نشطة</Text><Text style={styles.recentText}>ستظهر تفاصيل طلبك وحالة السفير هنا فور التأكيد.</Text></View></View></ScrollView><PremiumCustomerNav active="home" onHome={() => navigateCustomer("home")} onOrders={() => navigateCustomer("orders")} onProfile={() => navigateCustomer("profile")} /></View>;
}

function DriverProfile({ name, phone, onBack, onLogout }: { name: string; phone?: string; onBack: () => void; onLogout: () => void }) {
  const insets = useSafeAreaInsets();
  return <ScrollView contentContainerStyle={[driverProfileStyles.driverProfileScroll, { paddingTop: Math.max(insets.top + 16, 28), paddingBottom: Math.max(insets.bottom + 32, 48) }]}><Pressable onPress={onBack} style={driverProfileStyles.driverProfileBack}><MaterialIcons name="arrow-forward" size={21} color={dark} /></Pressable><View style={driverProfileStyles.driverProfileAvatar}><Text style={driverProfileStyles.driverProfileAvatarText}>{name.trim().charAt(0) || "س"}</Text></View><Text style={driverProfileStyles.driverProfileTitle}>حساب السفير</Text><Text style={driverProfileStyles.driverProfileSubtitle}>بيانات الحساب المسجلة في OPTIMUS X</Text><View style={driverProfileStyles.driverProfileCard}><View style={driverProfileStyles.driverProfileRow}><Text style={driverProfileStyles.driverProfileLabel}>الاسم</Text><Text style={driverProfileStyles.driverProfileValue}>{name || "غير مسجل"}</Text></View><View style={driverProfileStyles.driverProfileDivider} /><View style={driverProfileStyles.driverProfileRow}><Text style={driverProfileStyles.driverProfileLabel}>رقم الهاتف</Text><Text style={driverProfileStyles.driverProfileValue}>{phone || "غير مسجل"}</Text></View><View style={driverProfileStyles.driverProfileDivider} /><View style={driverProfileStyles.driverProfileRow}><Text style={driverProfileStyles.driverProfileLabel}>نوع الحساب</Text><Text style={driverProfileStyles.driverProfileValue}>سفير</Text></View></View><Text style={driverProfileStyles.driverProfileHint}>هذه الصفحة لعرض البيانات وتسجيل الخروج فقط. لا يمكن تعديل بيانات الاعتماد من التطبيق.</Text><Pressable onPress={onLogout} style={driverProfileStyles.driverProfileLogout}><MaterialIcons name="logout" size={19} color="#B91C1C" /><Text style={driverProfileStyles.driverProfileLogoutText}>تسجيل الخروج</Text></Pressable></ScrollView>;
}

function Driver({ name, phone, onTripActivity, onLogout }: { name: string; phone?: string; onTripActivity: (active: boolean) => void; onLogout: () => void }) {
  ...
  const logoutDriver = async () => { if (accessToken) await clearLocation.mutateAsync({ accessToken }).catch(() => undefined); await onLogout(); };
  ...
  useEffect(() => {
    ...
    const subscription = BackHandler.addEventListener("hardwareBackPress", () => {
      ...
      if (page === "profile") { setPage("home"); return true; }
...
export function Jarbou3App() {
  ...
  const submitForm = () => {
    ...
    if (role === "driver" && !isMatchingTeamDriver && (!onboardingPersonalPhoto || !onboardingIdentityPhoto || !onboardingVehiclePhoto)) {
      Alert.alert("وثائق السفير مطلوبة", "التقط الصورة الشخصية وصورة الهوية وصورة الدراجة النارية قبل إرسال طلب المراجعة.");
      return;
    }
    setPhone(normalizedPhone);
    submitOnboarding.mutate({ fullName: name.trim(), phone: normalizedPhone, requestedRole: role, vehicleType: role === "driver" ? vehicleType : undefined, personalPhoto: role === "driver" && !isMatchingTeamDriver ? onboardingPersonalPhoto ?? undefined : undefined, identityPhoto: role === "driver" && !isMatchingTeamDriver ? onboardingIdentityPhoto ?? undefined : undefined, vehiclePhoto: role === "driver" && !isMatchingTeamDriver ? onboardingVehiclePhoto ?? undefined : undefined });
  };
  const verifyCode = () => {
...
const driverProfileStyles = StyleSheet.create({ driverProfileScroll: { flexGrow: 1, alignItems: 'center', paddingHorizontal: 20, gap: 14, backgroundColor: '#F5F7F5' }, driverProfileBack: { alignSelf: 'flex-start', width: 44, height: 44, borderRadius: 14, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' }, driverProfileAvatar: { width: 82, height: 82, borderRadius: 28, backgroundColor: '#EA580C', alignItems: 'center', justifyContent: 'center', marginTop: 8 }, driverProfileAvatarText: { color: '#FFFFFF', fontSize: 34, fontWeight: '900' }, driverProfileTitle: { color: dark, fontSize: 24, fontWeight: '900' }, driverProfileSubtitle: { color: '#78716C', fontSize: 12, textAlign: 'center' }, driverProfileCard: { width: '100%', backgroundColor: '#FFFFFF', borderRadius: 20, padding: 16, marginTop: 8 }, driverProfileRow: { minHeight: 40, flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', gap: 12 }, driverProfileLabel: { color: '#A8A29E', fontSize: 11, fontWeight: '700' }, driverProfileValue: { color: dark, fontSize: 13, fontWeight: '900', textAlign: 'right', flexShrink: 1 }, driverProfileDivider: { height: 1, backgroundColor: '#FFF7ED', marginVertical: 8 }, driverProfileHint: { color: '#A8A29E', fontSize: 11, lineHeight: 18, textAlign: 'right', width: '100%' }, driverProfileLogout: { width: '100%', minHeight: 52, borderRadius: 16, borderWidth: 1, borderColor: '#FECACA', flexDirection: 'row-reverse', gap: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FEF2F2' }, driverProfileLogoutText: { color: '#B91C1C', fontSize: 14, fontWeight: '900' }, driverProfileButton: { width: 42, height: 42, borderRadius: 14, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' } });
