...
type HamaAddressResult = { label: string; latitude: number; longitude: number };
...
function Customer() {
  ...
  const addressSearch = trpc.jarbou3.searchHamaAddresses.useQuery({ query: addressQuery.trim().length >= 2 ? addressQuery.trim() : "حماة" }, { enabled: false });
