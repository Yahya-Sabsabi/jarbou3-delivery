...
function Jarbou3Marker({ point, label, color, title, mouse = false }: { point: MapPoint; label: string; color: string; title: string; mouse?: boolean }) {
  return <Marker coordinate={point} title={title} anchor={{ x: 0.5, y: 0.5 }}><View style={[styles.marker, mouse && styles.mouseMarker, { backgroundColor: color }]}>{mouse ? <Image source={require("@/assets/images/icon.png")} style={styles.mouseIcon} contentFit="cover" /> : <Text style={styles.markerText}>{label}</Text>}</View></Marker>;
}
