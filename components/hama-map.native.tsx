...
export function HamaMap({ compact = false, source, destination, driverLocation, selecting, onSelect, readOnly = false }: HamaMapProps) {
  return (
    <View style={[styles.map, compact && styles.mapCompact]}>
      <MapView mapType="none" style={styles.nativeMap} initialRegion={{ ...HAMA_CENTER, latitudeDelta: 0.14, longitudeDelta: 0.14 }} onPress={(event) => { const point = event.nativeEvent.coordinate; if (!readOnly && selecting && isInsideHama(point.latitude, point.longitude)) onSelect?.(point); }}>
